...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Kiểm tra xem form có được submit không
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $email = $_POST['email'];
...
if ($stmt->execute()) {
    ...
    $user_id = $stmt->insert_id; // Lấy ID của người dùng vừa đăng ký

    // Log để kiểm tra referrer_id và user_id
    error_log("User registered: user_id = $user_id, referrer_id = $referrer_id");

    // Cập nhật hoa hồng cho người giới thiệu nếu có
    if ($referrer_id) {
