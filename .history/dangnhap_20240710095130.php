...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        // Thêm ghi nhật ký để kiểm tra thông tin người dùng và mật khẩu đã nhập
        error_log('User found: ' . json_encode($user));
        
        if (password_verify($password, $user['password'])) {
            $_SESSION['user_id'] = $user['id'];

            // Kiểm tra trạng thái mua hàng của người dùng
            $user_id = $user['id'];
            ...
            if ($result->num_rows > 0) {
                // Người dùng đã mua hàng, chuyển hướng đến trang chi tiết
                echo json_encode(['status' => 'ordered']);
            } else {
                // Người dùng chưa mua hàng, chuyển hướng đến trang thanh toán
                echo json_encode(['status' => 'not_ordered']);
            }
        } else {
            // Thêm ghi nhật ký nếu mật khẩu không khớp
            error_log('Password does not match for user: ' . $user['id']);
            echo json_encode(['status' => 'failed']);
        }
    } else {
        // Thêm ghi nhật ký nếu không tìm thấy người dùng
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            loginForm.addEventListener('submit', function(event) {
                ...
                //Tạo đối tượng FormData và gửi yêu cầu AJAX:
